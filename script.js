...
function processVoiceCommand(text) {
    ...
    // 🌟 ADVANCED QUERY: Sabse zyada udhar kis par hai?
    ...
    // 🌟 ADVANCED QUERY: Aaj kitna udhar gaya?
    ...
    // 🌟 ADVANCED QUERY: Aaj kitna jama hua?
    ...
    // 🌟 ADVANCED QUERY: Total grahak kitne hain?
    ...
    if (type === 'delete' || type === 'balance') {
        if (!khataRegister[foundName]) { let err = `Bhai, "${foundName}" ka khata dukan mein nahi hai.`; showVoiceResponse("🤖 " + err); speakText(err); return; }
    }
    if (type === 'balance') {
    ...
    // 🌟 PROBLEM 1 SOLVED: DIRECT ENTRY (BINA POPUP KE)
    ...
    // Voice aur Toast Message se bata dena
    let successMsg = `${foundName} ke khate mein ${money} rupiye ${type === 'udhar' ? 'likh diye gaye hain' : 'jama kar liye gaye hain'}.`;
    speakText(successMsg);
    // User ko ek temporary message dikhane ke liye (Toast jaisa)
    let stat = document.getElementById("micStatus");
    stat.style.color = "#10b981"; // Green color
    stat.innerText = `✅ ${successMsg}`;
    setTimeout(() => { stat.style.color = "#4b5563"; stat.innerText = ""; }, 4000);
}
...
function deleteTransactionItem(grahak, id) {
    ...
    showConfirmModal(`Kya aap "${b.samaan}" delete karna chahte hain?`, () => {
